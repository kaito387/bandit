...
def _write_generated_case(
    ...
    payload = generate_full_binary_case.generate_case(
        k=k,
        s=s,
        ratio=ratio,
        algo=algo,
...
def _build_group_name(k: int, s: int, rounds: int, group: str | None) -> str:
    return group or f"tree-shape-sweep-S{s}K{k}-T{rounds}"
...
def _run_one_trial(args: argparse.Namespace) -> None:
    ...
    run_args = argparse.Namespace(
        ...
        sweep_group=_build_group_name(args.K, args.S, args.rounds, args.wandb_group),
        wandb_name=run_name,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--S", type=int, default=3, help="branching factor S")
    ...
    parser.add_argument("--ratio", type=float, required=True, help="single ratio in [0, 1] from sweep config")
